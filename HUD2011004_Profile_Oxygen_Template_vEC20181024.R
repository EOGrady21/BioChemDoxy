...
# read data ####
df_data_raw <- read.csv("HUD2011004_Oxygen_Rpt.csv")
...
df_data_raw <- df_data_raw %>%
  ...
  #dplyr::mutate(., winkler_avg=rowMeans(cbind(.$winkler1, .$winkler2))) %>%
...
# Calculate new SOC for Secondary CTD Oxygen (Sensor # 0042) ####
soc2 <- .42560
...
write.csv(x = df_data_filtered, file = 'HUD2011004_Oxy_corrected.csv')
