...
Event <- qatDF$Station.number
Sample_ID <- qatDF$Sample.id
...
oxyFile <- "2000009_oxy.xls"

# Read in the discrete oxygen data.
#colClassO <- c("character","integer",rep("numeric",4),rep("character",4),rep("numeric",3),"character")
#oxyDF <- read.xls(oxyFile, 1, colIndex=1:14, colClasses=colClassO, startRow = 9)
#rm(colClassO)
#edit to prep from BCD source
oxyDF <- readxl::read_xls(oxyFile)

#source('prepare_oxyCalib.R')
#oxyDF <- Dat$HUD12042          #sub list for each cruise
...
oxyID <- as.numeric(oxyDF$`Sample ID Number`)
...
write.table(oxyReportDF, file = "HUD2000009_Oxygen_Rpt.csv", quote = FALSE, sep = ",", row.names = FALSE, na = "")
...
save.image(file = 'HUD200009_Prepared_Data.RData')

cat('The prepared data has been saved to the file HUD2012042_Prepared_Data.R')
