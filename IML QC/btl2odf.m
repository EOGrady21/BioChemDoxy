...
%
%Modifications
%  - Ajouts pour traiter un fichier BTL sans référence à un CTD (ex: estuaire79_80)
% CL, 07-Nov-2005
%  - Dans create_empty_CTD, si heure == NaN, on inscrit 00:00:00 et on met
%  un commentaire dans l'événement (ex: algues toxiques)
% CL, 27-Apr-2006
...
types=[header(1,J_ech) header(1,J_zbtl) header(1,setdiff(J:end,[J_ech J_zbtl]))];
...
for j=1:length(names)
   try
      ...
      k=intersect(strmatchi(names{j},key.code),strmatchi(types{j},key.type));
      if length(k)>1
          k=intersect(strmatchi(names{j},key.code,'exact'),strmatchi(types{j},key.type));
      end
      ...
      disp(names{j})
      b=names{j}(1:findstr(names{j},'_'));
      if isempty(b)
          k=[];
      else
          k=intersect(strmatchi(names{j}(1:findstr(names{j},'_')),key.code),strmatchi(types{j},key.type));
      end
...
%   if isempty(isnan(dat1(ndat,strmatchi('PRES',names1))==1))
   ...
   lat=str2num(num2str(str2num(char(data(ndat,J))),'%.4f'));
   if length(find(lat==str2num(num2str(getvalue(CTD,'Initial_Latitude'),'%.4f'))))~=length(ndat)
      error([CTD.filename ' and bottle sample do not have the same latitude'])
   end
   J=strmatchi('Longitude',header(2,:));
   lon=str2num(num2str(str2num(char(data(ndat,J))),'%.4f'));
   if length(find(lon==str2num(num2str(getvalue(CTD,'Initial_Longitude'),'%.4f'))))~=length(ndat)
   ...
   while j<length(codes)
      ...
      %History
      c=strmatchi(names{j},btlLIST(nbtl(j)).method,'exact');
      if isempty(c)
   ...
   if exist(fileT,'file')==2     
      ...
      B(m).Quality_Header.Quality_Comments = ...
       ...
       disp('!!!! Fichier QC_Tests_performed.txt inexistant. Les fichiers ODF-BOTL n''auront pas de QUALITY_HEADER')
   end
...
try
    ...
    heure = '00:00:00';
    c = 'L`heure d`échantillonnage est inconnue';
    disp(c)
end
...
IQposition = strmatchi('Q-position',names);
if ~isempty(IQposition) & str2num(data{IQposition})
    S.Event_Header.Event_Comments{length(S.Event_Header.Event_Comments)+1,1} = ['La qualité de la position géographique est de ' data{IQposition}];
end
IQdatetime = strmatchi('Q-date/time',names);
if ~isempty(IQdatetime) & str2num(data{IQdatetime})
    S.Event_Header.Event_Comments{length(S.Event_Header.Event_Comments)+1,1} = ['La qualité de la date/heure d`échantillonnage est de ' data{IQdatetime}];
end
