...
xlabel('TEMP (°C)','FontWeight','bold')

axes(ax1)
temp=S.temp(Id,:);
temp2=temp;
hold on
if isfield(S,'Qtemp') & ~isempty(S.Qtemp)
   Qtemp=S.Qtemp(Id,:);
   for j=1:size(temp,2)
      It0 = find(Qtemp(:,j)==2);
      t0 = plot(temp(It0,j),deph(It0),'^','color',[0.1 0.1 0.6],'Markerfacecolor','y');
      It2 = find(Qtemp(:,j)==3 | Qtemp(:,j)==4);
      t2 = plot(temp(It2,j),deph(It2),'o','color',[0.6 0.1 0.1],'Markerfacecolor','r');
      It3 = find(Qtemp(:,j)==7);
      t3 = plot(temp(It3,j),deph(It3),'s','color',[0.1 0.6 0.1],'Markerfacecolor','g');
      It4 = find(Qtemp(:,j)~=3 | Qtemp(:,j)~=4);
      temp2(It2,j)=nan;
   end
end
t1 = plot(temp,deph,'.k');
temp2 = S.CTD_temp;
t4 = plot(temp2(isnan(temp2)==0),deph(isnan(temp2)==0),'-','Color',[0.8 0.1 0.1]);

ax1=gca;
lx=get(ax1,'XLim'); lx(1)=max([lx(1)-1]); lx(2)=lx(2)+1;
ly=get(ax1,'YLim'); ly(1)=0;
set(ax1,'XLim',lx,'YLim',ly,'YDir','reverse','XAxisLocation','top','XminorTick','on','YminorTick','on','Box','on');
xlabel('Temperature','FontWeight','bold'); ylabel('Depth (m)','FontWeight','bold');
grid on
...
xlabel('Salinity','FontWeight','bold'); % ylabel('Depth (m)');
grid on

%Axes 3: Dissolved Oxygen
...
ax3=gca;
lx=get(ax3,'XLim'); lx(1)=max([lx(1)-1; 0]); lx(2)=lx(2)+1;
...
ts=plot(psal2,temp2,'.-','Color',[0.8 0.1 0.1]);
