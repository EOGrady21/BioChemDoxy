...
% optph='phph' or 'pht_' data (default phph)
% Notes:
%  -O2 data are always selected from upcasts.
...
% CL, 15-sep-2009
% - ajout optph pour traiter PHPH ou PHT_
% CL, 05-Fev-2015
...
case 'fbtl'
   ...
   for i=1:size(B,2)
       if isfield(B(i).btl.data,'ALT__01')
           B(i).btl.data.ALT__01(B(i).btl.data.ALT__01>30)=nan;
       end
   end
end
...
other_var = {'cntr','cndc','flor','tran','psar','spar','cdom','trb_','alt_'};
for i=1:size(Q,2)
    for j=1:size(other_var,2)
        Q(i).(other_var{j}) = getvalue(S(i),[upper(other_var{j}) '_01']);
        if isempty(Q(i).(other_var{j}))
            Q(i).(other_var{j}) = nan*Q(i).pres;
        end
        if strcmp(other_var{j},'alt_')
            Q(i).alt_(Q(i).alt_>30)=nan;
        end
...
other={'FLOR','DOXY','TRAN','PSAR','SPAR','CDOM','PHPH','TRB_','ALT_'};

for i=1:length(other)
  if eval(['~isnan(nanmin(cat(1,Q.' lower(other{i}) ')))'])
...
for i=1:size(Q,2)
   ...
   case 'fbtl'
      ...
      %Traitement comme dans seawifs  
      %sample
      uniq=[-99];
      BTL.data.PRES_01(:,1)=round(BTL.data.PRES_01(:,1)*10)/10;
      K=find(BTL.data.PRES_01(:,1)>U.deph(J(k))-3 & BTL.data.PRES_01(:,1)<U.deph(J(k))+3);
      uniq=U.sample(J(k));
      if length(K)>1
         K=find(abs(BTL.data.PRES_01(:,1)-U.deph(J(k)))==min(abs(BTL.data.PRES_01(:,1)-U.deph(J(k)))));
      end
      if length(K)>1
         K=setdiff(K,KK);
         K=K(end);
         disp(['Two entries for sample ' num2str(uniq) ' in ' BTL.filename ' at ' num2str(BTL.data.PRES_01(K,1),'%.1f') ' dbar']) 
      end
      KK=[KK;K];
   	  try,
   		CTD.sample(end+1,1)=uniq;
      ...
      %other sensors
      for o=1:length(other)
          Iother = strmatchi(other{o},h2);
          if ~isempty(Iother)
      	      for kk=1:length(Iother)
                  switch other{o}
                  case 'DOXY'
                      switch optoxy
                      case 'downcast'
                  ...
                  otherwise
                      if isfield(BTL.data,[other{o} '_' num2str(kk,'%02d')])
                          CTD.data(end,Iother(kk)-8)=BTL.data.([other{o} '_' num2str(kk,'%02d')])(K,1);
                      else
                          CTD.data(end,Iother(kk)-8)=nan(size(K));
                      end
                  end
              end
          end
   ...
   otherwise
      ...
      case 'cntr'
            ...
            Z.phph=getvalue(SS,[optph '_01']);
            Z.trb_=getvalue(SS,'TRB__01');
            Z.alt_=getvalue(SS,'ALT__01');
      ...
      if ~isempty(L)
        ...
        for o=1:length(other)
            switch(other{o})
            case 'DOXY'
                if ~isempty(strmatchi(other{o},h2))
                    O=find(A.pres==max(A.pres));
                    LO2=find(A.pres(1:O)>=nanmean(A.pres(L))-1 & A.pres(1:O)<=nanmean(A.pres(L))+1);
                    CTD.data(end,strmatchi(other{o},h2)-8)=nanmean(A.(lower(other{o}))(LO2));
                end
            otherwise
                if ~isempty(strmatchi(other{o},h2))
                    CTD.data(end,strmatchi(other{o},h2)-8)=nanmean(A.(lower(other{o}))(L));
                end
            end
        end
      else
         CTD.data(end+1,1:end)=nan;
      end
...
%Unique filename-depth (2 sample numbers)
file_zb = strcat(CTD.filename,num2str(CTD.zbouteille));
[u_file_zb,ia] = unique(file_zb);
if size(file_zb,1)~=size(u_file_zb)
    ib=setdiff((1:size(file_zb,1)),sort(ia));
    for b=1:length(ib)
        disp(['File ' CTD.filename{ib(b)} ' at depth ' num2str(CTD.zbouteille(ib(b)),'%.1f') ' has two different unique numbers associated to'])
    end
end
%Labo data
%unique number
...
switch upper(optph)
case 'PHT_'
    h{2}=strrep(h{2},'PHPH','PHT_');
    h{3}=strrep(h{3},'(NBS scale)','(Total scale)');
end
