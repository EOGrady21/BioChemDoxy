...
#Emily Chisholm, March 2019
#modified slightly for Oxygen Recovery Project
#plotting historical biochem data, main focus on winkler data
...
plot_profiles <- function(xlist) {
  ...
  xrange=c(min(na.omit(data$value)),max(na.omit(data$value))) #added na omit to get proper min max
  
  # CASE 1: if there is data for 1st element (ctd), then plot ctd and the rest if exists
  #if (!is.null(l$v1)){ #changed logical to check for nulls first
  if (l$v1[1]>0) {
  ...
  #if (is.null(l$v1) & !is.null(l$v2)){ #changed to screen for null values (empty data)
  if (l$v1[1]==0 & l$v2[1]>0) {
    ...
    if (xlist$label=="Oxygen" & (l$v2[1]>0 | l$v3[1]>0)) {  #!is.null(l$v2) | !is.null(l$v3)
      legend("bottomright",legend=c("CTD", "Electrode","Winkler"),pch=c(1,16,15),col=c("black", "dodgerblue","blue"), lty=c(1,1,3),bty="n")
    }
  ...
  #if ( is.null(l$v1) & is.null(l$v2)){ #changed to screen for nulls
 if (l$v1[1]==0 & l$v2[1]==0) {
