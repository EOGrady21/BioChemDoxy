...
# parameters for QC
vars <- read.csv('C:/Users/ChisholmE/Documents/BIOCHEM/IML QC/BCD_IML_map_upd.csv')
...
for (i in 1:length(fe)) {
  ...
  event=sprintf("%03d",as.numeric(fe[i])) # event padded with leading zeroes like 013 or 001
  ...
  dp=d[,c("DIS_SAMPLE_KEY_VALUE","DIS_HEADER_START_DEPTH","DIS_DETAIL_DATA_VAL","DATA_TYPE_METHOD","DIS_DETAIL_DATA_QC_CODE")]
 ...
 ud=unique(dp$depth)
  ...
  plot(t$value,-as.numeric(t$depth), main="Temperature",type="b", xlab="Temperature [C]", ylab="Depth [m]")
  }
  
  # if there is no temp plot empty box
  if (length(temp)==0) {
    #php=rep(8,length(ud))
  ...
  if (length(flag)>0){
    ...
    cat("-> QC repot:")
    cat("\n")
    print(rbind(qcrep[ltp1,]), row.names=F)   #qcrep_ctd[ltp,],
  ...
  #make numeric
  o3$depth <- as.numeric(o3$depth)
  o3$value <- as.numeric(o3$value)
  
  #get correct unit on plot
  if (length(o2w) > 0){
  if (dp[o2w, 'method'][1] == "O2_Winkler_Molar"){
    xlab = "Oxy [mmol/m**3]"
  } else{
    xlab = "Oxy [ml/l]"
  }
  ...
  # make a list
  olist=list(v1=o1,v2=o2,v3=o3,label="Oxygen",xlab=xlab)
...
flag=which(dp$qc[c(o2ctd,o2e,o2w)]>1) # find flag index
  if (length(flag)>0){
   ...
   #ltp=which(qcrep_ctd$Filename %in% paste0(mission,"_",event) )
    ...
    cat("-> QC repot:")
    cat("\n")
    print(rbind(qcrep[ltp1,]), row.names=F,na="")   #qcrep_ctd[ltp,],
  ...
  if (length(ph)==0) {
    php=rep(8,length(ud))
    plot(php,-as.numeric(ud),pch=' ', xlab="pH",ylab="Depth[m]", xlim=c(7,8.8), main="pH_CTD") }
  ...
  if (length(flag)>0){
    ...
    # make a dataframe of flagged data
    if (length(c1$sample_key)> 0 & length(c2$sample_key) >0){
    cflagged=merge(c1,c2,by="sample_key", all.x=T)
    }
    if (length(c1$sample_key)> 0 & length(c2$sample_key) == 0){ 
      cflagged = c1
    }
    if (length(c2$sample_key)> 0 & length(c1$sample_key) == 0){ 
      cflagged = c2
    }
  ...
  if (length(c(chl,chlctd))==0) {
    php=rep(2,length(ud))
    plot(php,-as.numeric(ud),pch=' ', xlab="Chl [mg/m3]",ylab="Depth[m]", xlim=c(0,5), main="Chlorophyll") }
...
if (length(nit)>0) {
  ...
  plot(as.numeric(n$value),-as.numeric(n$depth), main="Nitrate", type="b", xlab="Nit [mmol/m3]", ylab="Depth[m]", pch=16,col="dodgerblue")
    
    flag=which(dp$qc[nit]>1) # find flag index
    if (length(flag)>0){
      clrs=flagcol2(dp$qc[nit][flag])
      points(as.numeric(dp$value[nit][flag]),-as.numeric(dp$depth[nit][flag]),cex=2,bg=clrs,pch=21) 
...
pho=grep("PO4_",d$DATA_TYPE_METHOD)
  ...
  plot(as.numeric(pp$value),-as.numeric(pp$depth), main="Phosphate", type="b", xlab="Pho [mmol/m3]", ylab="Depth[m]", pch=16,col="dodgerblue")
    
    flag=which(dp$qc[pho]>1) # find flag index
    if (length(flag)>0){
      clrs=flagcol2(dp$qc[pho][flag])
      points(as.numeric(dp$value[pho][flag]),-as.numeric(dp$depth[pho][flag]),cex=2,bg=clrs,pch=21)  
...
sil=grep("SiO4_",d$DATA_TYPE_METHOD)
  ...
  plot(as.numeric(si$value),-as.numeric(si$depth), main="Silicate", type="b", xlab="Sil [mmol/m3]", ylab="Depth[m]", pch=16, col="dodgerblue")
    
    flag=which(dp$qc[sil]>1) # find flag index
    if (length(flag)>0){
      clrs=flagcol2(dp$qc[sil][flag])
      points(as.numeric(dp$value[sil][flag]),-as.numeric(dp$depth[sil][flag]),cex=2,bg=clrs,pch=21) 
