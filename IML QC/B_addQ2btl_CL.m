...
% CL, 19-Nov-2009
% - Correction aux niveaux des entêtes des colonnes de QC
% AL, 02-oct-2018
...
%Variable names
names_B  = {'TEMP_RT'  ,'SSTP_BK'  ,'PSAL_BS','SSAL_BS','SIGT_0','OXY_0','OXYM_0','CHL_0','NO2_0','NO3_0','NOX_0','PO4_0','Si_0' ,'pH_0' ,'pHT_0','ALKY_0','QCFF'};  
names_BQ = {'Q_TEMP_RT','Q_SSTP_BK','Q_PSAL' ,'Q_SSAL' ,'Q_SIGT','Q_OXY','Q_OXYM' ,'Q_CHL','Q_NO2','Q_NO3','Q_NOX','Q_PO4','Q_Si' ,'Q_pH' ,'Q_pHT','Q_ALKY','QCFF'};
names_Q  = {'temp'     ,'temp'     ,'psal'   ,'psal'   ,'sigt'  ,'doxy' ,'doxy'  ,'cphl' ,'ntri' ,'ntrz' ,'ntrz' ,'phos' ,'slca' ,'phph' ,'phph' ,'alky'  ,'QCFF'};
names_QQ = {'Qtemp'    ,'Qtemp'    ,'Qpsal'  ,'Qpsal'  ,'Qsigt' ,'Qdoxy','Qdoxy' ,'Qcphl','Qntri','Qntrz','Qntrz','Qphos','Qslca','Qphph','Qphph','Qalky' ,'QCFF'};
...
while ok
    ...
    if ~isempty(K) && Qflag(K) && isempty(KQ)
        cellQ = [{S.data{1,j}};names_BQ(K);{'((none))'};repmat({'0'},size(S.data,1)-3,1)];
