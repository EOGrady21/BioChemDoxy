...
for ax=1:length(axes_no)
    ...
    set(a1,'Box','off','XLim',lx,'Tickdir','out','YMinorTick','on');
    set(a1,'Xtick',(1:length(files)),'XTickLabel','')
    if ax==4
    ...
    a2=gca;
    v1=line(ind,var1,'Color','k','Parent',a2);
    v2=line(ind,var2,'Color',[0.1 0.1 0.5],'Parent',a2);
    set(v1,'Marker','s','MarkerFaceColor',[0.8 0.2 0.2],'MarkerSize',4);
    set(v2,'Marker','^','MarkerFaceColor',[0.2 0.8 0.2],'MarkerSize',4);
    set(a2,'Tickdir','out','XTick',[],'YMinorTick','on','XLim',lx);
    if ~isnan(nanmax([var1(:); var2(:)])) 
       set(a2,'Ylim',[0 ceil(nanmax([var1(:); var2(:)]))]);
    else
       ylim=get(a2,'YLim');
       set(a2,'Ylim',[0 ylim(2)]);
    end
    ...
    text(lx(1)+e1/3,ly(1)+ly(2)/3,'Depth (m)','Fontweight','bold',...
