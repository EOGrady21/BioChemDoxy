function y = medfilt1_CL(x,n,blksz)
%MEDFILT1_CL - One-dimensional median filter with modified start and end of series
%   Y = MEDFILT1_CL(X,N) returns the output of the order N, one dimensional 
...
%
%   MEDFILT1_CL(X,N,BLKSZ) uses a for-loop to compute BLKSZ ("block size") 
...
%   Processing Toolbox.
%
%Uses first and endding values to fill the series before median 
%filter is applied.
%Caroline Lafleur, Ocean Sciences, DFO, 02-Dec-1999
...
X = [x(1)*ones(1,m) x(:)' x(end)*ones(1,m)];
y = zeros(1,nx);
