...
for i=1:size(L.data,2)
   ...
   if  strcmp(lower(L.header{i}(end-1:end)),'xx') | ~isempty(t)
      I=intersect(strmatch(lower(L.header{i}(1:end-2)),lower(key.code),'exact'),strmatch('labo',key.type));
      if isempty(I)
          I=intersect(strmatch(lower(L.header{i}(1:end-2)),lower(key.code),'exact'),strmatch('terrain',key.type));
      end
      %if isempty(I)
      %    I=strmatch(lower(L.header{i}(1:end-3)),lower(key.code),'exact');
      %end
   elseif isempty(t)
      I=intersect(strmatch(lower(L.header{i}),lower(key.code),'exact'),strmatch('labo',key.type));
      if isempty(I)
          I=intersect(strmatch(lower(L.header{i}),lower(key.code),'exact'),strmatch('terrain',key.type));
      end
   end
