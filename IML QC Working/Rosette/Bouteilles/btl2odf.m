...
if ~isempty(IQposition) & str2num(data{IQposition})
    S.Event_Header.Event_Comments{length(S.Event_Header.Event_Comments)+1} = ['La qualité de la position géographique est de ' data{IQposition}];
end
IQdatetime = strmatchi('Q_date/time',names);
if ~isempty(IQdatetime) & str2num(data{IQdatetime})
    S.Event_Header.Event_Comments{length(S.Event_Header.Event_Comments)+1} = ['La qualité de la date/heure d`échantillonnage est de ' data{IQdatetime}];
end
