...
if ~isempty(uni)
   [U.sample,U.file,U.deph] = textread('unique_no.txt','%f %s %f','endofline','\r\n');
else
...
%other={'FLOR','FLOR','DOXY','TRAN','PSAR','SPAR','CDOM','PHPH'};
...
for i=1:size(Q,2) 
   ...
   case 'fbtl'
      ...
      %FLOR_01
      Iflor = strmatchi('flor',h2);
      if ~isempty(Iflor) && length(Iflor)==1
      	try, CTD.data(end,Iflor(1)-8)=BTL.data.FLOR_01(K,1);
      	catch, CTD.data(end,Iflor(2)-8)=nan; end
      %FLOR_02
      elseif ~isempty(Iflor) && length(Iflor)==2
      	try, CTD.data(end,Iflor(1)-8)=BTL.data.FLOR_01(K,1);
             CTD.data(end,Iflor(2)-8)=BTL.data.FLOR_02(k,1);
      	catch, CTD.data(end,Iflor(1)-8)=nan;
               CTD.data(end,Iflor(2)-8)=nan;end
      end
      %DOXY
      if ~isempty(strmatchi('doxy',h2))
        try
            q=find(A.pres>BTL.data.PRES_01(K,1)-1.0 & A.pres<BTL.data.PRES_01(K,1)+1.0);
       		if ~isempty(q), CTD.data(end,strmatchi('doxy',h2)-8)=nanmean(A.doxy(q));
        		else, datestr('1'); end
     		catch
   ...
   otherwise
      ...
      if ~isempty(L)
         ...
         if ~isempty(strmatchi('spar',h2))
      	   try, CTD.data(end,strmatchi('spar',h2)-8)=nanmean(A.psar(L));
            catch, CTD.data(end,strmatchi('spar',h2)-8)=nan; end
         end
         if ~isempty(strmatchi('cdom',h2))
...
if ~isempty(LABO.filename)
   for i=1:size(LABO.filename,1)
      J=strmatchi(LABO.filename{i},CTD.filename);
      if isempty(J)
