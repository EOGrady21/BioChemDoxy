...
%  opt='cntr': takes the data at specified scan numbers in upcast
...
if ~isempty(uni)
	fid=fopen('unique_no.txt','rt');
	disp('Reading unique_no.txt ....')
	n=0;
	while feof(fid)==0
   	n=n+1;
   	line=fgetl(fid);
  		U.sample(n)=sscanf(line,'%f %*s %*f');
   	U.file{n}=sscanf(line,'%*f %s %*f');
   	U.deph(n)=sscanf(line,'%*f %*s %f');
	end
	fclose(fid);
else
...
btl=dir('btlscan.txt');
if ~isempty(btl)
	fid=fopen('btlscan.txt','rt');
	disp('Reading btlscan.txt ....') 
	n=0;
	while feof(fid)==0
   	n=n+1;
   	line=fgetl(fid);
  		N.sample(n)=sscanf(line,'%f %*s %*f %*f %*f');
   	N.file{n}=sscanf(line,'%*f %s %*f %*f %*f');
   	N.deph(n)=sscanf(line,'%*f %*s %f %*f %*f');
   	N.startscan(n)=sscanf(line,'%*f %*s %*f %f %*f');
   	N.endscan(n)=sscanf(line,'%*f %*s %*f %*f %f');
	end
...
for i=1:size(Q,2)
   Q(i).cntr=getvalue(S(i),'CNTR_01');
   Q(i).cndc=getvalue(S(i),'CNDC_01');
  	Q(i).flor=getvalue(S(i),'FLOR_01');
  	Q(i).doxy=getvalue(S(i),'DOXY_01');
  	Q(i).tran=getvalue(S(i),'TRAN_01');
  	Q(i).psar=getvalue(S(i),'PSAR_01');
    Q(i).oxv_=getvalue(S(i),'OXV__01');
   Q(i).comments=getvalue(S(i),'Event_Comments');  
end
...
otherwise
	...
	[Qfiles{1:size(Q,2)}]=deal(Q.filename);
	I=[];
	for i=1:length(Bfiles)
 	  J=strmatchi(Bfiles(i),Qfiles);
	  I=[I J];
	end
	Q=Q(I);
	S=S(I);
...
other={'FLOR','DOXY','TRAN','PSAR','OXV_'};
...
for i=1:size(Q,2) 
   ...
   case 'fbtl'
      ...
      %PSAR
      if ~isempty(strmatchi('par',h2))
      	try, CTD.data(end,strmatchi('par',h2)-8)=BTL.data.PSAR_01(k,1);
      	catch, CTD.data(end,strmatchi('par',h2)-8)=nan;	end	
      end
      %OXV_
      if ~isempty(strmatchi('oxv_',h2))
        switch optoxy
      case 'downcast'
        %oxygen from downcast
        try
            K=find(A.pres>BTL.data.PRES_01(k,1)-1.0 & A.pres<BTL.data.PRES_01(k,1)+1.0);
        	if ~isempty(K), CTD.data(end,strmatchi('oxv_',h2)-8)=nanmean(A.oxv_(K));
            else, datestr('1'); end
        catch
        	CTD.data(end,strmatchi('oxv_',h2)-8)=nan;
        end
      case 'upcast'
        %oxygen from BTL-file data
    	try, CTD.data(end,strmatchi('oxv_',h2)-8)=BTL.data.OXV__01(k,1);
      	catch, CTD.data(end,strmatchi('oxv_',h2)-8)=nan;	end	
      end
      end
      end
   %opt 'deph' and 'cntr'
   otherwise
      ...
      case 'cntr'
      	...
      	if isempty(L)
         	Z=split_odf(S(i),2);
         	Z=create_std(Z,'inconnu_10');
         	Z=setQto1(Z,'flag');
         	L=find(Z.pres>N.deph(I(j))-1.0 & Z.pres<N.deph(I(j))+1.0);
         	disp(['CNTR=' num2str(N.startscan(I(j))) ' not found in ' ...
               Z.filename(1:end-4) ' -> Pressure depth used instead'])
      	end
      ...
      if ~isempty(L)
         ...
         if ~isempty(strmatchi('par',h2))
      	   try, CTD.data(end,strmatchi('par',h2)-8)=nanmean(A.psar(L));
            catch, CTD.data(end,strmatchi('par',h2)-8)=nan; end
         end
         if ~isempty(strmatchi('oxv_',h2))
   			try, CTD.data(end,strmatchi('oxv_',h2)-8)=nanmean(A.oxv_(L));
            catch, CTD.data(end,strmatchi('oxv_',h2)-8)=nan; end
         end
      else
